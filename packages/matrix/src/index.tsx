 ...
 *
 * MAS needs the 32-byte config/cookie encryption key generated by its
 * config schema (a bare `secrets` section is mandatory in MAS 1.24.0 —
 * `secrets.encryption`, 64 hex chars / 32 bytes) plus the MAS↔homeserver
 * shared secret (`matrix.secret` — "Missing `secret` or `secret_file`"
 * otherwise). Provision BOTH in the backend bundle path
 * `<mount>/<secrets path>/${name}/mas` under keys `encryption_key`
 * (`openssl rand -hex 32`) and `homeserver_secret` (any random string);
 * the package renders a `<name>-mas-secrets` bundle automatically when a
 * secrets backend is in scope. Without a backend, pre-create that Secret
 * yourself — the pod stays in CreateContainerConfigError with
 * `Secret "${name}-mas-secrets" not found` until it exists.
 *
 * @example
...
function buildMasConfig(
  ...
  return {
    ...
    // schema gap after secrets). Schema per crates/config/src/sections/
    // matrix.rs @ v1.24.0: the root-level `matrix` key itself is required
    // (`homeserver` has a serde default) — and the flattened `secret` field
    // has none: TryFrom bails "Missing `secret` or `secret_file`". That
    // secret is shared between MAS and the homeserver (admin-API identity);
    // like secrets.encryption it is a SECRET → template placeholder,
    // substituted by the render-config init container from the provisioned
    // `<name>-mas-secrets` bundle (key `homeserver_secret`).
    matrix: {
      homeserver: serverName,
      // Leaving `endpoint` untouched falls back to MAS's upstream default
      // http://localhost:8008/ for homeserver admin-API calls — unreachable
      // from this pod. Pin the in-cluster synapse client API Service.
      endpoint: `http://${name}-synapse:8008`,
    ...
    database: {
      ...
      // upstream, and a password placeholder inside an embedded connection
      // string trips the noPlaintextSecrets guardrail (it has no
      // __PLACEHOLDER__ exemption for URIs, but does for key/value lines).
      // Splitting also drops the $(MASPASSWORD) env-expansion placeholder —
      // Kubernetes never expands $(VAR) inside mounted file CONTENTS, so the
      // URI form would have authenticate with the literal string.
      host: `${name}-mas-db-rw`,
      port: 5432,
      database: 'mas',
      username: 'mas',
      // Placeholder, substituted by the render-config init container with
      // the CNPG secret's password (single-quote-wrapped with '' escaping —
      // same discipline as the synapse render). The ConfigMap below is a
      // TEMPLATE and never carries a real secret.
      password: '__MAS_DB_PASSWORD__',
      ssl_mode: 'prefer',
    },
    // MAS 1.24.0 requires a root-level `secrets` section (dogfood smoke
    // round 4: "Error: missing field `secrets`" — the next schema gap after
    // binds/public_base). Schema per crates/config/src/sections/secrets.rs
    // @ v1.24.0: SecretsConfig REQUIRES the flattened `encryption` key —
    // 64 hex chars decoding to the 32-byte cookie-encryption key (or
    // `encryption_file`); `keys`/`keys_dir` are optional. The key is a
    // SECRET: it rides the template as a placeholder, substituted by the
    // render-config init container from the provisioned
    // `<name>-mas-secrets` bundle — never a ConfigMap literal.
    secrets: { encryption: '__MAS_ENCRYPTION_KEY__' },
    http: {
        ...
        providers: [
          {
            ...
            // Placeholder — also substituted at init-render time from the
            // keycloak-oidc bundle (mounted into the init container). The old
            // '$MAS_OIDC_CLIENT_SECRET' env reference was dead on arrival:
            // MAS/figment interpolates no $-references inside config values,
            // so the literal string would have synced into the DB as the OIDC
            // client secret.
...
function buildMasEnv(): EnvVar[] {
  // Secrets do NOT ride env into the main container — they are substituted
  // into the rendered config by the render-config init container (see the
  // Deployment, synapse's pattern). MAS_CONFIG mirrors the explicit
  // `--config` arg and covers mas-cli subcommands run by operators.
  return [
    {
      // The config-file env var per crates/cli/src/commands/mod.rs
      // @ v1.24.0 is `MAS_CONFIG` (not MAS_CONFIG_FILE)
      name: 'MAS_CONFIG',
...
function masDeployment(opts: {
  ...
  replicas: number
  masVersion: string
  /** Same image the synapse Deployment pulls — its python3 does the render */
  renderImage: string
  keycloakSecretName: string | undefined
}): ReturnType<typeof jsx> {
  const { name, namespace, replicas, masVersion, renderImage, keycloakSecretName } = opts

  // The pinned MAS image is distroless (gcr.io/distroless/cc-debian13
  // per the upstream Dockerfile) — no shell, no python — so the config
  // render cannot reuse it like synapse's render-config does. The synapse
  // image carries python3 and is already pulled on every cluster running
  // this stack; plain str.replace, no sed regex escaping, safe against
  // special chars. All substituted values are single-quote-wrapped with ''
  // YAML escaping so secret content (spaces, #, :, quotes) cannot corrupt
  // the rendered config. Config template (ConfigMap, placeholder-laden) +
  // provisioned secrets → /config/config.yaml on the config emptyDir.
  // Unrendered placeholders fail the init container loudly: a half-rendered
  // config would otherwise sync placeholder strings into MAS's
  // database-synced settings.
  const renderConfigCommand = [
    'python3',
    '-c',
    "import pathlib,sys; t=pathlib.Path('/template/config.yaml.tpl').read_text(); " +
      "k=pathlib.Path('/secrets/mas/encryption_key').read_text().strip(); " +
      "s=pathlib.Path('/secrets/mas/homeserver_secret').read_text().strip(); " +
      "p=pathlib.Path('/secrets/db/password').read_text().strip(); " +
      't=t.replace(\'__MAS_ENCRYPTION_KEY__\', "\'" + k.replace("\'", "\'\'") + "\'"); ' +
      't=t.replace(\'__MAS_HOMESERVER_SECRET__\', "\'" + s.replace("\'", "\'\'") + "\'"); ' +
      't=t.replace(\'__MAS_DB_PASSWORD__\', "\'" + p.replace("\'", "\'\'") + "\'"); ' +
      "o=pathlib.Path('/secrets/oidc/clientSecret'); " +
      't=t.replace(\'__MAS_OIDC_CLIENT_SECRET__\', "\'" + o.read_text().strip().replace("\'", "\'\'") + "\'") if o.exists() else t; ' +
      "left=[m for m in ('__MAS_ENCRYPTION_KEY__','__MAS_HOMESERVER_SECRET__','__MAS_DB_PASSWORD__','__MAS_OIDC_CLIENT_SECRET__') if m in t]; " +
      "sys.exit('unrendered placeholders: ' + ', '.join(left)) if left else None; " +
      "pathlib.Path('/config/config.yaml').write_text(t)",
  ]
    ...
    spec: {
      ...
      template: {
        ...
        spec: {
          ...
          initContainers: [
            {
              ...
              volumeMounts: [
                { name: 'config-template', mountPath: '/template', readOnly: true },
                { name: 'mas-secrets', mountPath: '/secrets/mas', readOnly: true },
                { name: 'db-credentials', mountPath: '/secrets/db', readOnly: true },
                ...(keycloakSecretName
                  ? [
                      {
          ...
          containers: [
            {
              ...
              env: buildMasEnv(),
              // /config carries the init-rendered config — the main
              // container never sees the raw secrets (encryption key, DB
              // password, OIDC client secret are baked into the rendered
              // file by the init container, then the process exits).
          ...
          volumes: [
            // CNPG generates the database credentials Secret (<cluster>-app);
            // the encryption key rides the provisioned <name>-mas-secrets
            // bundle (openbao sync when a secrets backend is configured —
            // key `encryption_key`) or a pre-created Secret by that name.
            // Only the render-config init container reads any of them.
            { name: 'db-credentials', secret: { secretName: `${name}-mas-db-app` } },
            { name: 'mas-secrets', secret: { secretName: `${name}-mas-secrets` } },
            ...(keycloakSecretName
              ? [
                  {
                    name: 'oidc-credentials',
                    secret: { secretName: keycloakSecretName },
                  } as never,
                ]
              : []),
            // Final rendered config (init-container output) + its template
            { name: 'config', emptyDir: {} },
            { name: 'config-template', configMap: { name: `${name}-mas-config` } },
          ],
        },
      },
    },
...
export function Matrix(props: MatrixProps) {
  ...
  if (hasBackend) {
    // MAS always deploys and the 1.24.0 schema always requires
    // secrets.encryption + matrix.secret — this bundle is NOT gated on sso
    secretSecretDefs.push({
      name: `${name}-mas-secrets`,
      path: `${secretProvider!.path ?? name}/${name}/mas`,
      keys: { encryption_key: 'encryption_key', homeserver_secret: 'homeserver_secret' },
    })
  ...
  resources.push(
    jsx('ConfigMap', {
      ...
      // TEMPLATE, not the final config: the encryption key (__MAS_ENCRYPTION_KEY__),
      // the DB password (__MAS_DB_PASSWORD__) and the OIDC client secret
      // (__MAS_OIDC_CLIENT_SECRET__) are substituted by the MAS Deployment's
      // render-config init container from the provisioned secrets. A real
      // secret must never land in this ConfigMap.
      data: {
        'config.yaml.tpl':
          '# Generated by @r8s/matrix — template (render-config substitutes __MAS_ENCRYPTION_KEY__/__MAS_DB_PASSWORD__[, __MAS_OIDC_CLIENT_SECRET__])\n' +
          toYaml(masConfig),
      },
    })
  )
  ...
  resources.push(
    masDeployment({
      ...
      // The MAS image is distroless — the render-config init container
      // borrows the synapse image's python3 (same pin)
      renderImage: `matrixdotorg/synapse:${synapseVersion}`,
      keycloakSecretName,
    })
  )
  ...
  if (Array.isArray(value)) {
    return value
      .map((item) => {
        ...
        // Primitive items go through yamlScalar too — they are scalar VALUES
        // like any other, and an unquoted YAML-1.1-ambiguous string gets
        // re-typed by the consumer's parser (smoke round 4: synapse inside a
        // PyYAML (YAML 1.1) image read `- ::` back as the dict {':': null},
        // then twisted listenTCP crashed with AttributeError: 'dict' object
...
// Strings that YAML 1.1 resolvers (PyYAML — the parser inside synapse/MAS
// images) re-type as boolean/null/number even though YAML 1.2 (js-yaml — what
// the unit tests parse with) leaves them strings. A single-quoted scalar is
// ALWAYS a string under both revisions, so when in doubt: quote.
const YAML11_AMBIGUOUS_SCALAR =
  /^(?:~|null|Null|NULL|true|True|TRUE|false|False|FALSE|yes|Yes|YES|no|No|NO|on|On|ON|off|Off|OFF|[-+]?\.(?:inf|Inf|INF|nan|NaN|NAN)|[-+]?(?:0[xX][0-9a-fA-F_]+|0[oO][0-7_]+|0[bB][01_]+|[0-9][0-9_]*(?:\.[0-9_]*)?(?:[eE][-+]?[0-9]+)?|\.[0-9_]+(?:[eE][-+]?[0-9]+)?))$/
...
function yamlScalar(value: unknown): string {
  ...
  if (
    YAML11_AMBIGUOUS_SCALAR.test(str) ||
    /[:#\s'"{}[\],&*!|>%@`]/.test(str) ||
    str.startsWith('$')
  ) {
