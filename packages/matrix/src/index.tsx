 ...
 * `secrets.encryption`, 64 hex chars / 32 bytes). Provision it in the
 * backend bundle path `<mount>/<secrets path>/${name}/mas` under key
 * `encryption_key` (e.g. `openssl rand -hex 32`); the package renders a
 * `<name>-mas-secrets` bundle automatically when a secrets backend is in
 * scope. Without a backend, pre-create that Secret yourself — the pod
 * stays in CreateContainerConfigError with `Secret "${name}-mas-secrets"
 * not found` until it exists.
 *
 * @example
...
function masDeployment(opts: {
  ...
  const renderConfigCommand = [
    ...
    "import pathlib,sys; t=pathlib.Path('/template/config.yaml.tpl').read_text(); " +
      ...
      "left=[m for m in ('__MAS_ENCRYPTION_KEY__','__MAS_DB_PASSWORD__','__MAS_OIDC_CLIENT_SECRET__') if m in t]; " +
...
export function Matrix(props: MatrixProps) {
  ...
  if (hasBackend) {
    ...
    // secrets.encryption — this bundle is NOT gated on sso
