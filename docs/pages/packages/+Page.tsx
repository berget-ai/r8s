    ...
    <div className="space-y-8">
      ...
      <div className="prose prose-invert max-w-none">
        <p className="text-base leading-relaxed">
          Each package declares the operators it depends on and exports typed components. Use them
          alone or combine them inside recipes for end-to-end infrastructure.
        </p>
      </div>
        ...
        {categories.map((category) => {
          ...
          return (
            <div key={category} className="space-y-4">
              <h2 className="text-xs text-moss uppercase tracking-wider font-medium">{category}</h2>
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        ...
                        <p className="text-xs font-mono text-cloud/50 mt-1">{pkg.name}</p>
                      </div>
                      <span className="text-cloud/40 text-sm">→</span>
                    </div>
                    <p className="text-cloud/70 text-sm leading-relaxed mb-4">{pkg.description}</p>
      ...
      <div className="mt-12 p-6 rounded-lg border border-white/10 bg-spruce/20">
        ...
        <p className="text-cloud/70 text-sm leading-relaxed">
          ...
          — like Database, WebService, and App — which combine packages and operators into complete
          infrastructure.
        </p>
      </div>
    </div>
