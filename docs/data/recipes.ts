    ...
    component: {
      ...
      props: [
        ...
        },
        {
          ...
          required: false,
          description:
            "Key inside the store's credentials Secret holding a velero-format cloud credentials file (`[default] aws_access_key_id = …`). Omit on workload-identity / IRSA clusters.",
        },
        {
          name: 'ttl',
          type: 'string',
    ...
    component: {
      ...
      props: [
        ...
        },
        {
          ...
          required: false,
          description:
            "Where database credentials come from. Default `'backend'`: when a provisioning secrets backend is configured on the Platform, the credentials secret (`<name>-db-credentials`) is provisioned through it (rotation → pod restarts) and CNPG adopts it via `bootstrap.initdb.secret` — see `databaseCredentialsRef` for the exact resolution contract. `'cnpg'` forces CNPG-managed bootstrap credentials **even with a backend** — the operator generates the secret in-cluster (incl. `fqdn-uri`), matching apps that reference the CNPG-generated secret directly. Without a backend (or under passive backends that render no referenceable credentials Secret) the contract is the CNPG-generated `<name>-app` secret: `bootstrap.initdb.secret` is omitted and the operator generates the credentials in-cluster. App packages resolve the password secret through `databaseCredentialsRef` — never hardcode a `-db-credentials` name.",
        },
        {
          name: 'children',
          type: 'unknown',
